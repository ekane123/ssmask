...
def make_kid(w_ind, l_ind, l0, final_h,
            ...
            ind_layer, cap_layer, ground_layer, detector_index):
    """
    ...
    ind_layer, ground_layer, detector_index: Layer numbers for all layers.
    ...
    R0 = pg.rectangle(size=(w_ind, l0), layer=ind_layer)
    ...
    CapPlate1 = pg.rectangle(size=(wC, hC), layer=cap_layer)
    CapPlate1.add_port(name='1', midpoint=(w_ind/2, 0), orientation=270)
    CapPlate2 = pg.rectangle(size=(wC, hC), layer=cap_layer)
    ...
    R2 = pg.rectangle(size=(l0_coupler_connector, w_coupler_connector), layer=cap_layer)
    R2.add_port(name='1', midpoint=(0, w_coupler_connector/2), orientation=180)
    R2.add_port(name='2', 
                midpoint=(l0_coupler_connector, w_coupler_connector/2), 
                orientation=0)

    R3 = pg.rectangle(size=(w_coupler, h_coupler), layer=cap_layer)
    ...
    R4 = pg.rectangle(size=(h_coupler1, w_coupler), layer=cap_layer)
...
def make_filterbank_with_kids(d_stubs, l_stubs, dist_from_mmwave_line,
                            filter_f0s, spacing, eps_eff, bend_radius, h_staple, 
                              l_open, w_mstrip, Qc_gaps, filter_layer,
                              w_inds, l_inds, l0, final_h,
                            h_caps, w_caps, gapC, w_coupler_connector, l0_coupler_connector,
                            coupling_ground_height, coupling_ground_width,
    ...
    for ii in range(len(h_caps)):
        ...
        Dkid = make_kid(w_ind, l_ind, l0, final_h,
            hcap, wcap, gapC, w_coupler_connector, l0_coupler_connector,
            coupling_ground_height, coupling_ground_width,
            coupling_ground_gap, wcoup, hcoup, distance_to_cpw,
            ind_layer, cap_layer, ground_layer, detector_index=ii)
