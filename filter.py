...
def make_filter(f0, eps_eff, bend_radius, h_staple, w_mstrip, Qc_gap):
    ...
    R2 = pg.Device('rect')
    points =  [(0, 0), (l_staple-arc0.xsize, 0), (l_staple-arc0.xsize, w_mstrip), (0, w_mstrip)]
    R2.add_polygon(points)
    R2.add_port(name = '1', midpoint = [0,w_mstrip/2], width = w_mstrip, orientation = 180)
    R2.add_port(name = '2', midpoint = [l_staple-arc0.xsize,w_mstrip/2], width = w_mstrip, orientation = 0)
